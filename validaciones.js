...
function utilidades(tiempo, sueldo) {
    if (tiempo < 1) {
        sueldo *= 1.05;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("calificacionForm").addEventListener("submit", (event) => {
        ...
        if (!validarNumero(cal1) || !validarNumero(cal2) || !validarNumero(cal3) || !validarNumero(calex) || !validarNumero(calt)) {
            alert("Ingrese calificaciones válidas");
            return;
        }
    ...
    document.getElementById("registroForm").addEventListener("submit", (event) => {
        ...
        if (!passwordRegex.test(password)) {
            alert("La contraseña debe contene 6 caracteres o mas, una letra mayúscula, una letra minúscula y un dígito");
            return;
        }

        if (!comentarios || comentarios.length > 50) {
            alert("Ingreso incorrecto, el campo comenterios es obligatorio y no pueden exceder los 50 caracteres");
            return;
        }
